...
const VentasDiarias = () => {
  ...
  const fetchVentasDiarias = async () => {
    try {
      const response = await axios.get(`${apiUrl}/api/remitos`);
      ...
      const fetchProductDetails = async (productoId) => {
        const response = await axios.get(`${apiUrl}/api/productos/${productoId}`);
  ...
  const handleDelete = async (fecha) => {
    ...
    if (window.confirm("¿Estás seguro de que deseas eliminar las ventas de este día?")) {
      try {
        await axios.delete(`${apiUrl}/api/remitos/fecha/${fecha}`);
        fetchVentasDiarias();
      } catch (error) {
